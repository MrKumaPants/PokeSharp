...
public class CrumbleLedgeBehavior : ScriptBase
{
    ...
    public override void Initialize(ScriptContext ctx)
    {
        ...
        Context.Logger.LogInformation(
            "Crumble ledge initialized: max={MaxJumps} jumps, direction={Direction}",
            _maxJumps,
            _ledgeDirection
        );
    }
    ...
    public override void RegisterEventHandlers(ScriptContext ctx)
    {
            ...
            (evt) =>
            {
                ...
                if (evt.Direction == oppositeDirection)
                {
                    Context.Logger.LogDebug(
                        $"Crumble ledge: Blocking opposite direction movement (dir={oppositeDirection})"
                    );
        ...
        On<MovementCompletedEvent>(
            (evt) =>
            {
                ...
                Context.Logger.LogInformation(
                    "Ledge jumped: {JumpCount}/{MaxJumps} times",
                    jumpCount,
                    _maxJumps
                );
    ...
    private void CrumbleLedge(Entity entity)
    {
        ...
        Context.Logger.LogWarning(
            "Ledge crumbled after {JumpCount} jumps at ({X}, {Y})",
            jumpCount,
            Context.TileData.X,
            Context.TileData.Y
        );
