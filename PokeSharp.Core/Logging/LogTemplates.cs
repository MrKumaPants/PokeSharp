...
public static class LogTemplates
{
    ...
    /// </summary>
    public static void LogFramePerformance(this ILogger logger, float avgMs, float fps, float minMs, float maxMs)
    {
        var fpsColor = fps >= 60 ? "green" : fps >= 30 ? "yellow" : "red";
        var message = $"[blue]⚡[/] Performance: [cyan]{avgMs:F1}ms[/] [{fpsColor}]{fps:F1} FPS[/] [dim]| Min: {minMs:F1}ms | Max: {maxMs:F1}ms[/]";
        logger.LogInformation(message);
    }
    ...
    /// </summary>
    public static void LogSystemPerformance(this ILogger logger, string systemName, double avgMs, double maxMs, long calls)
    {
        var avgColor = avgMs > 1.67 ? "red" : avgMs > 0.84 ? "yellow" : "green";
        var message = $"[blue]│[/] [cyan]{systemName,-25}[/] [{avgColor}]{avgMs,6:F2}ms[/] [dim]avg[/] [yellow]{maxMs,6:F2}ms[/] [dim]max[/] [grey]│[/] [dim]{calls} calls[/]";
        logger.LogInformation(message);
    }
    ...
    /// </summary>
    public static void LogSlowSystem(this ILogger logger, string systemName, double timeMs, double percent)
    {
        ...
        var message = $"{icon} {label} [cyan bold]{systemName}[/] [{timeColor}]{timeMs:F2}ms[/] [dim]│[/] [{percentColor}]{percent:F1}%[/] [dim]of frame[/]";
        logger.LogWarning(message);
    }
    ...
    /// </summary>
    public static void LogOperationFailedWithRecovery(this ILogger logger, string operation, string recovery)
    {
    ...
    private static string FormatDetails(params (string key, object value)[] details)
    {
        ...
        var formatted = string.Join(", ", details.Select(d => $"[dim]{EscapeMarkup(d.key)}:[/] [grey]{EscapeMarkup(d.value.ToString() ?? "")}[/]"));
    ...
    private static string FormatComponents(params (string key, object value)[] components)
    {
        ...
        var formatted = string.Join("[dim],[/] ", components.Select(c => $"[grey]{EscapeMarkup(c.key)}[/]"));
        return $" [dim][[{formatted}]][/]";
    }
}
