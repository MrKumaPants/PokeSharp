...
public class Phase4MigrationTests : IDisposable
{
    ...
    private readonly MockApiProvider _mockApis;
    ...
    public void MultipleScripts_AttachToSameTile_WorkCorrectly()
    {
        ...
        Entity tileEntity = _world.Create();
        var tileContext = new ScriptContext(
            _world,
            tileEntity,
            NullLogger.Instance,
            _mockApis,
        ...
        var evt = new TileSteppedOnEvent
        {
            ...
            FromDirection = 0,
            Elevation = 0,
            BehaviorFlags = TileBehaviorFlags.ForcesMovement,
        };
    ...
    private ScriptService CreateMockScriptService()
    {
        ...
        ILogger<ScriptService> logger = loggerFactory.CreateLogger<ScriptService>();

        return new ScriptService("/scripts", logger, loggerFactory, _mockApis, _eventBus, _world);
    }
    ...
    public void JumpTile_DirectionalBehavior_WorksAfterMigration()
    {
        ...
        var validJumpEvent = new TileSteppedOnEvent
        {
            ...
            Elevation = 0,
            BehaviorFlags = TileBehaviorFlags.None,
        };
    ...
    public void JumpTile_AllDirections_WorkIndependently()
    {
        ...
        (Direction, string, int)[] directions = new[]
        {
        ...
        foreach ((Direction jumpDir, string tileType, int fromDir) in directions)
        {
    ...
    public void ImpassableTile_BlocksMovement_AfterMigration()
    {
        ...
        var evt = new TileSteppedOnEvent
        {
            ...
            FromDirection = 0,
            Elevation = 0,
            BehaviorFlags = TileBehaviorFlags.BlocksMovement,
        };
        _eventBus.Publish(evt);
    ...
    public void NPCScript_MovementPattern_WorksAfterMigration()
    {
        ...
        var npcScript = new MigratedNPCPatrolScript(
            new[]
            {
                new Vector2(10, 10),
                new Vector2(15, 10),
        ...
        var npcContext = new ScriptContext(
            _world,
            _npcEntity,
            NullLogger.Instance,
            _mockApis,
    ...
    public async Task MigratedScript_CanBeReloaded_WithoutErrors()
    {
        ...
        ScriptService scriptService = CreateMockScriptService();
    ...
    public void HotReloadTest_VerifiesEventResubscription()
    {
        ...
        _output.WriteLine(
            $"✓ Original script did not receive new event (count stayed at {testScript.EventCount})"
        );
...
public class MigratedJumpTileScript : ScriptBase
{
    ...
    public override void RegisterEventHandlers(ScriptContext ctx)
    {
        On<TileSteppedOnEvent>(evt =>
        {
            ...
            int[] validDirections = GetValidApproachDirections(_jumpDirection);

            if (!validDirections.Contains(evt.FromDirection))
            {
                evt.PreventDefault("Cannot jump from this direction");
                ctx.Logger.LogDebug(
                    $"Jump tile: Blocked approach from direction {evt.FromDirection}"
                );
            }
            else
            {
...
public class MigratedNPCPatrolScript : ScriptBase
{
    ...
    public void ExecutePatrol()
    {
        ...
        Vector2 targetPos = _patrolPoints[_currentPoint];

        Context.Logger.LogDebug(
            $"NPC patrolling to point {_currentPoint}: ({targetPos.X}, {targetPos.Y})"
        );
            ...
            Publish(
                new MovementCompletedEvent
                {
                    ...
                    OldPosition = (
                        (int)
                            _patrolPoints[
                                (_currentPoint - 1 + _patrolPoints.Length) % _patrolPoints.Length
                            ].X,
                        (int)
                            _patrolPoints[
                                (_currentPoint - 1 + _patrolPoints.Length) % _patrolPoints.Length
...
public class MigratedCustomEventPublisherScript : ScriptBase
{
    ...
    {
        Publish(new CustomMigrationEvent { Message = "Test event from migrated script" });
    }
}
...
public class MigratedLoggingScript : ScriptBase
{
    ...
    public override void RegisterEventHandlers(ScriptContext ctx)
    {
        On<TileSteppedOnEvent>(evt =>
        {
            LoggedEvent = true;
            ctx.Logger.LogDebug(
                $"Logging script: Entity stepped on tile at ({evt.TileX}, {evt.TileY})"
            );
        });
    }
}
...
public class MockApiProvider : IScriptingApiProvider
{
    ...
    public MockApiProvider(World world, IEventBus eventBus)
    {
        ...
        Dialogue = new MockDialogueApiService(
            _world,
            _eventBus,
            NullLogger<MockDialogueApiService>.Instance
        );
        Effects = new EffectApiService(
            _world,
            _eventBus,
            NullLogger<EffectApiService>.Instance,
            new MockGameTimeService()
        );
    }
...
{
    public MockDialogueApiService(
        World world,
        IEventBus eventBus,
        ILogger<MockDialogueApiService> logger
    )
